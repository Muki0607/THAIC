...
function ui.DrawRepText(ttfname, title, text, pos, x, y, alpha, timer, shake)
    ...
    if title == "" then
        ...
        yos = (#text - 1) * ui.menu.sc_pr_line_height * 0.5
        SetImageState(title, "", Color(255 * alpha, 255, 255, 255))
...
function lstg_ui:drawInfo1()
    ...
    local RenderImgList = {
        ...
        { "line_3", 109 + w.scrr, 359, 0, 1, 1 },
        { "line_4", 109 + w.scrr, 321, 0, 1, 1 },
        { "line_5", 109 + w.scrr, 247 + b_dy, 0, 1, 1 },
        { "line_6", 109 + w.scrr, 224 + b_dy, 0, 1, 1 },
        { "line_7", 109 + w.scrr, 202 + b_dy, 0, 1, 1 },
        ...
        { "hint.power", 39 + w.scrr, 253 + b_dy, 0, 0.5, 0.5 },
        { "hint.point", 39 + w.scrr, 230 + b_dy, 0, 0.5, 0.5 },
        { "hint.graze", 54 + w.scrr, 208 + b_dy, 0, 0.5, 0.5 }
    }
    local s = stage.current_stage
    local timer = s.timer
    local alplat
    ...
    RenderText("score1", string.format("%d.    /4.    ", math.floor(lstg.var.power / 100)), 204 + w.scrr, 262 + b_dy, 0.4, "right")
    RenderText("score1", string.format("      %d%d        00", math.floor((lstg.var.power % 100) / 10), lstg.var.power % 10), 205 + w.scrr, 258.5 + b_dy, 0.3, "right")
    RenderScore("score2", lstg.var.pointrate, 204 + w.scrr, 239 + b_dy, 0.4, "right")
    SetFontState("score3", "", Color(alplat, 173, 173, 173))
    RenderText("score3", string.format("%d", lstg.var.graze), 204 + w.scrr, 216 + b_dy, 0.4, "right")
